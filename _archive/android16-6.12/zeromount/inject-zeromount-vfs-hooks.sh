...
state == 1 && /^[[:space:]]*int error;/ && !injected {
    ...
    print "\t\tint zm_ret = zeromount_stat_hook(dfd, filename->uptr, stat, request_mask, flags);"
...
/
    ...
    # 6.6 signature: int generic_permission(struct mnt_idmap *idmap, struct inode *inode, int mask)
    sed -i '/^int generic_permission(struct mnt_idmap \*idmap, struct inode \*inode,$/,/^}$/{
/^{$/,/int ret;/{
/int ret;/a\
\
#ifdef CONFIG_ZEROMOUNT\
...
\
    ...
    # 6.6 signature: int inode_permission(struct mnt_idmap *idmap, struct inode *inode, int mask)
    sed -i '/^int inode_permission(struct mnt_idmap \*idmap,$/,/^}$/{
/^{$/,/int retval;/{
/int retval;/a\
\
#ifdef CONFIG_ZEROMOUNT\
...
state == 1 && /return -EBADF;/ && !skip_done {
    ...
    print ""
    print "#ifdef CONFIG_ZEROMOUNT"
    print "\tif (fd_file(f)->f_pos >= ZEROMOUNT_MAGIC_POS) {"
    print "\t\terror = 0;"
...
state == 1 && /error = buf\.error;/ && !inject_done {
    ...
    print "\t\tzeromount_inject_dents(fd_file(f), (void __user **)&dirent, &count, &fd_file(f)->f_pos);"
...
state == 1 && /error = buf\.error;/ && !inject_done {
    ...
    print "skip_real_iterate:"
    print "\tif (error >= 0 && !signal_pending(current)) {"
    print "\t\tzeromount_inject_dents64(fd_file(f), (void __user **)&dirent, &count, &fd_file(f)->f_pos);"
    print "\t\terror = initial_count - count;"
    print "\t\tgoto zm_out;"
    print "\t}"
    print "#endif"
...
#include <linux/zeromount.h>\
    ...
    # d_path function signature and local vars (6.6, same as 5.15):
    #   char *d_path(const struct path *path, char *buf, int buflen)
    #   {
    #       DECLARE_BUFFER(b, buf, buflen);
    #       struct path root;
    #
    # We inject after "struct path root;" to spoof virtual paths for injected files
    # 6.6 uses DECLARE_BUFFER (no local "res" variable), so the hook declares its own
    # res from buf+buflen.
...
#include <linux/zeromount.h>\
    ...
    # Inject at opening brace of function body — signature-agnostic approach,
    # matching on the function name line then waiting for the opening brace.
    awk '
