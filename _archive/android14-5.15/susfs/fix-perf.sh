...
# Three changes to readdir.c in the patch:
...
if grep -q 'ilookup(buf->sb, ino)' "$GKI_PATCH" 2>/dev/null; then
    ...
    !injected_shared_fn && /^\+extern bool susfs_is_inode_sus_path\(struct inode \*inode\);/ {
        print
    ...
    /^\+\tinode = ilookup\(buf->sb, ino\);/ {
        # Consume the entire block until orig_flow: or #endif
