    ...
    if grep -q 'AS_FLAGS_OPEN_REDIRECT_ALL' "$namei"; then
        ...
        /^#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT/ {
            ...
            if (is_redirect_block && !already_replaced) {
                ...
                print "\t\t\t\tset_nameidata(&nd, dfd, fake_pathname, NULL);"
...
# 6.6: do_mkdirat takes struct filename *name (not const char __user *)
patch_unicode_namei() {
    ...
    # do_mkdirat — 6.6 uses struct filename *name
...
\t}\
    ...
    # do_symlinkat — 6.6 uses struct filename *to
...
\t}\
    ...
    # do_linkat — 6.6 uses struct filename *new
...
# 6.6: vfs_statx takes struct filename *, use ->uptr for user ptr
patch_unicode_stat() {
    ...
    # vfs_statx — 6.6 uses struct filename *, access user ptr via ->uptr
