...
#include <linux/zeromount.h>\
    ...
    # 6.6: vfs_statx takes struct filename *, use ->uptr for user pointer
...
/
    ...
    # generic_permission() — 6.6: 3 params (mnt_idmap, inode, mask)
...
\
    ...
    # inode_permission() — 6.6: 3 params (mnt_idmap, inode, mask)
...
#include <linux/zeromount.h>\
    ...
    # 6.6 uses DECLARE_BUFFER (no local "res" variable), so the hook
    # declares its own res from buf+buflen
    awk '
