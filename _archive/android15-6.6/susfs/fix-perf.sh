...
#   N2 - Wire name registration into add_sus_path
#
# 6.6 readdir.c has 5 filldir variants (return bool, not int):
...
# -- P1: Inline fast-path for link_path_walk SUS_PATH check --
if grep -A1 'dentry = nd->path.dentry;' "$GKI_PATCH" 2>/dev/null | grep -q 'susfs_is_inode_sus_path(idmap, dentry->d_inode)'; then
...
# 6.6 filldir variants return bool (not int). The ilookup replacement logic
# is identical to 5.10 except the early-return value is false instead of 0
# for non-onedir variants (buf->error is not set on bool return).
if grep -q 'ilookup(buf->sb, ino)' "$GKI_PATCH" 2>/dev/null; then
    ...
    in_readdir && !injected_shared_fn && /^\+extern bool susfs_is_inode_sus_path\(struct mnt_idmap\*/ {
        print
    ...
    # 6.6 filldir returns bool: true = continue, false = stop
...
echo "=== fix-perf done ($fix_count fixes) ==="
