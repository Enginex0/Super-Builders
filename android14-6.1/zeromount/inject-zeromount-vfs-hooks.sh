...
static inline int zeromount_stat_hook(int dfd, struct filename *filename, \
                                      ...
                                      unsigned int flags) {\
    if (filename && filename->uptr) {\
        char kname[NAME_MAX + 1];\
        long copied = strncpy_from_user(kname, filename->uptr, sizeof(kname));\
        if (copied > 0 && kname[0] != '"'"'/'"'"') {\
...
/
    ...
    sed -i '/^int generic_permission(struct user_namespace \*mnt_userns, struct inode \*inode,$/,/^}$/{
/^{$/,/int ret;/{
/int ret;/a\
\
#ifdef CONFIG_ZEROMOUNT\
...
\
    ...
    sed -i '/^int inode_permission(struct user_namespace \*mnt_userns,$/,/^}$/{
/^{$/,/int retval;/{
/int retval;/a\
\
#ifdef CONFIG_ZEROMOUNT\
...
#include <linux/zeromount.h>\
    ...
    # d_path function signature and local vars (5.15):
    ...
    # 5.15 removed the local "res" variable (uses DECLARE_BUFFER instead),
    # so the hook declares its own res from buf+buflen.
