...
\t\tgoto out_copy_to_user;\
\t}\
\n\tnew_entry->mnt_id = mnt->mnt_id;\
\tspin_lock(&susfs_spin_lock_hidden_mount);\
\thash_add(HIDDEN_MOUNT_HLIST, &new_entry->node, new_entry->mnt_id);\
\tspin_unlock(&susfs_spin_lock_hidden_mount);\
\n\tSUSFS_LOGI("hide_mount: mnt_id=%d path='"'"'%s'"'"' added to hidden list\\n",\
\t           mnt->mnt_id, info.mount_point);\
\tpath_put(&p);\
\tinfo.err = 0;\
\nout_copy_to_user:\
...
\
bool susfs_is_mount_hidden(int mnt_id)\
{\
\tstruct st_susfs_hide_mount_hlist *entry;\
\n\thash_for_each_possible(HIDDEN_MOUNT_HLIST, entry, node, mnt_id) {\
